...
class nerAPI:
    ...
    def showMorp(self):
        ...
        print(strnum)
        morpnum = len(self.sentence[0]['NE']) #총형태소갯수
        print(morpnum) 
        ...
        with open('morpAPI.txt','w', encoding="utf-8") as make_file:
            for i in range(0, strnum):
              for j in range(0,morpnum-1):
                data =""
                pprint(self.sentence[i]['NE'][j]['type'])
                 
                pprint(self.sentence[i]['NE'][j]['text'])
                json.dump(self.sentence[i]['NE'][j]['type'],make_file,ensure_ascii=False, indent=4)
                json.dump(self.sentence[i]['NE'][j]['text'],make_file,ensure_ascii=False, indent=4)
...
if __name__ == "__main__":
  hi = nerAPI("C:\\Users\\sge\\python\\example.txt")
  hi.showMorp()