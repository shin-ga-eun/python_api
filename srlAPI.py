...
class srlAPI:
    ...
    def showSRL(self):
        ...
        print(strnum)
        analnum = len(self.result[0]['SRL']) #문장별 분석 갯수 
        print(analnum)

      
        with open('srlAPI.txt','w', encoding="utf-8") as make_file:
            for i in range(strnum):
                for j in range(analnum):
                    str=''
                    str = self.result[i]['SRL'][j]['verb']+" = "
                    for k in range(len(self.result[i]['SRL'][j]['argument'])):
                        argustr = ''
                        argustr += self.result[i]['SRL'][j]['argument'][k]['text'] +" , "+self.result[i]['SRL'][j]['argument'][k]['type']

                        json.dump(str+argustr, make_file, ensure_ascii=False, indent=4)


        #텍스트결과파일 원본 -> 편집된 텍스트파일로 변환
        myFile = open("C:\\Users\\sge\\python\\srlAPI.txt","r", encoding="utf-8")
        text = myFile.read()
        text = text.replace("\"\""," / ")
        text=text.replace("\"","")
        print(text)
...
res = srlAPI("C:\\Users\\sge\\python\\example.txt")
res.showSRL()